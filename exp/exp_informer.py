...
class Exp_Informer(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.model=='informer' or self.args.model=='informerstack':
            ...
            # Initiate model (either Informer or InformerStack) with args
    ...
    def _get_data(self, flag):
        ...
        # Create loader (first dim = batch_size)
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        # Create masking, only mask the predict sequence, shape[-1] = no_input_features
        if self.args.padding==0:
            dec_inp = torch.zeros([batch_y.shape[0], self.args.pred_len, batch_y.shape[-1]]).float()
        elif self.args.padding==1:
            dec_inp = torch.ones([batch_y.shape[0], self.args.pred_len, batch_y.shape[-1]]).float()
            
        # Concat mask to the decoder start tokens to create full length of decoder
